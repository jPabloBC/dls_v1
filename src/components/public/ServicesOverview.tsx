    ...
    <section className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {services.map((service) => (
          <article key={service.title} className="flex h-full flex-col rounded-lg border border-[#D6D9DC] bg-white p-6 shadow-sm">
            <div className="mb-5 h-1.5 w-14 rounded-full bg-[#C81010]" />
            <h3 className="text-lg font-semibold text-[#4A4F55]">{service.title}</h3>
            <p className="mt-3 text-sm leading-6 text-[#4A4F55]">{service.description}</p>
          </article>
        ))}
      </div>
    </section>
